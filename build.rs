...
fn main() {
    ...
    #[cfg(any(feature = "java_wrapper", feature = "cpp_wrapper"))]
    {
        ...
        #[cfg(feature = "cpp_wrapper")]
        {
            ...
            let cpp_cfg = CppConfig::new(cpp_output_dir, "mag_calib".into())
        ...
        #[cfg(feature = "java_wrapper")]
        {
            let interface_android_logger_filepath =
                ffi_source_folder.join("interface_android_logger.rs");
            ...
            if ["aarch64-linux-android", "armv7-linux-androideabi"].contains(&target.as_str()) {
                //android
                ...
                gen_java_binding(&target, &[include_dirs], &[jni_h_path], &jni_c_headers_rs)
                    .unwrap();
            } else {
                // host system is the target
            ...
            let java_cfg = JavaConfig::new(java_output_dir, "co.cariboulabs.mag_calib".into())
            ...
            swig_gen.expand_many(
                "mag_calib_java",
                &[interface_filepath, interface_android_logger_filepath],
...
#[cfg(feature = "java_wrapper")]
fn search_file_in_directory_recursive<P: AsRef<Path>>(
    dirs: &[P],
    file: &str,
) -> Result<PathBuf, ()> {
    for dir in dirs {
        ...
        for entry in
            fs::read_dir(&dir).expect(&format!("Can not read directory: {}", &dir.display()))
        {
